...
export default function EstudiantilesPage() {
  ...
  useEffect(() => {
    fetch('/api/packages')
      .then(res => res.json())
      .then(data => {
        // Filter only estudiantiles packages
        const estudiantilesPackages = data.filter((pkg: Package) => pkg.type === 'PAQUETE ESTUDIANTILES')
        setPackages(estudiantilesPackages)
        setFilteredPackages(estudiantilesPackages)
      })
      ...
      <div className="bg-gradient-to-br from-[#2C3E5C] to-[#3A4F6F] text-white py-16">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-xl text-white/90 max-w-2xl">
            Viajes diseñados especialmente para estudiantes con precios accesibles y experiencias únicas
          </p>
        </div>
      </div>
              ...
              <Card className="border-0 shadow-md">
                <CardContent className="p-12 text-center">
                  <p className="text-xl text-slate-600 mb-4">
                    No se encontraron paquetes estudiantiles con los filtros seleccionados
                  </p>
